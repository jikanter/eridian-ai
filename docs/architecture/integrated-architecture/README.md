...
- [`SPEC-astrophage.md`](SPEC-astrophage.md) — Cross-repo spec for **astrophage**, the wire-level record/replay/cache/mock tool forked out of eridian (the `eridian-replay` substrate of in-repo `ADR-0005`/`SPEC-003`, promoted to its own repo). Owns the seams that span aichat ↔ [brief](https://github.com/jikanter/brief) ↔ harness: `base_url` + correlation-header coupling to aichat, brief's deferred `cassettes:`/`## Fixtures` declarative field, and committed-cassette portability. Defers the wire contract to `SPEC-003`.