...
```bash
ls -1 src/knowledge/ | grep -E "^(cli|compile|edp|mod|restore|store|tags)\.rs$"
```
...
```bash
grep -E "^pub (struct|enum) |^pub fn" src/knowledge/edp.rs src/knowledge/tags.rs | head -20
```

```output
src/knowledge/edp.rs:pub struct FactId(String);
src/knowledge/edp.rs:pub struct SourceAnchor {
src/knowledge/edp.rs:pub enum EdgeKind {
src/knowledge/edp.rs:pub struct EdgeRef {
src/knowledge/edp.rs:pub struct EntityDescriptionPair {
src/knowledge/tags.rs:pub struct Tag {
src/knowledge/tags.rs:pub struct TagSchema {
```

## 25C — AEVS restore-check ladder

```bash
grep -E "^pub fn|^pub enum|^pub struct" src/knowledge/restore.rs
```
...
```bash
grep -E "pub const (MANIFEST_FILE|FACTS_FILE|EDGES_FILE|SCHEMA_FILE)|pub fn (create|load|save|append_edge|remove_facts_by_source|tag_index|outbound_edges|set_schema)|pub fn append_fact\(" src/knowledge/store.rs
```
...
```bash
grep -E "^pub fn |^pub struct |^pub const" src/knowledge/compile.rs | head -15
```
...
```bash
./target/debug/aichat --help 2>&1 | grep -A1 -E -- "--knowledge-(compile|list|stat|show)\b"
```
