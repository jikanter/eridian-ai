...
@test "phase27: --knowledge-reflect on missing KB errors clearly" {
  run "$AICHAT_BIN" --knowledge-reflect "kb-that-does-not-exist-phase27"
  [ "$status" -ne 0 ]
...
@test "phase27: --knowledge-curate requires existing KB" {
  run "$AICHAT_BIN" --knowledge-curate "kb-that-does-not-exist-phase27"
  [ "$status" -ne 0 ]
}

@test "phase27: --knowledge-candidates requires --knowledge-curate" {
  run "$AICHAT_BIN" --knowledge-candidates /tmp/candidates.json
  [ "$status" -ne 0 ]
