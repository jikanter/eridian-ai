...
**Next Step Implementation Note (2026-06-09)**:  For the implementation thread: the load-bearing decision is §9.4 (the off-diagonal (backing × facet) presets). Phases 28/29 will harden the RoleLike → Entity trait shape by accident if built before 52. 52A (trait rename + `facets()`) and 52B (facet taxonomy + `--dry-run` surfacing) are **shipped**; 52C (collapse the variant-specific `SessionEntity` / `EntityRef` resolution branches onto the `Entity` trait, **backing-gates-ownership** as the single resolution invariant) is in flight on its own branch. **Phase 42 (A–D) is now complete** — the SPEC-001 trace keystone is live on the request path. The full stack (`src/utils/trace_spec/`): ULID session ids, the 17-variant event envelope, the `LineSink`/`TraceSender` dedicated OS writer thread (bounded `sync_channel` + in-band `trace.dropped`), the `env_subset` redaction gate, the content-addressed blob store (`blob.rs`: SHA-256 + sharded `blobs/ab/cd/<hex>` + write-once), record-time auth-header stripping (key-independent `messages_hash`), the lifecycle coverage layer (`layout.rs` SPEC §1 paths + `manifest.jsonl`; `session.rs` `TraceSession` orchestrator), and the 42D surface (`wiring.rs`: `--trace`/`AICHAT_TRACE` opt-in, `TraceSession` minted in `call_react` emitting the full lifecycle, `X-Eridian-Session-Id` stamped at the single `retry::send` chokepoint) — all with **zero new dependencies** and verified end-to-end. Three deliberate carry-overs: the ad-hoc 8F/8G emitter still coexists (a later cleanup retires it); SPEC §1's default-on is held back as an Ask-First behavior change (tracing stays opt-in); and 42D's `provider.*` payloads are reconstructed from pre-send *intent*, not wire bytes — now formalized as **[Phase 42E](roadmap/phase-42e-overview.md)** (transport-boundary capture, in-process per `EVAL-001` §5, no mitmproxy). **52D is now shipped** — each keystone `session.start` carries `entity_id` + the resolved facet token set (the stable Phase 49 attribution key), additive at `schema_version` `0.1`. **The next concrete steps are 52C** (collapse the variant-specific resolution branches onto the `Entity` trait — unblocks the agent *owned*-facet view 52D records against the synthesized role) **and 42E** (provider wire fidelity). Phase 42 also unblocks the test harness (43), trace projections/training (44), and astrophage correlation (45D/46). The parallel Epic 10 successor remains **Phase 28** (agent composability).
...
| **15 Observability Keystone** | 42 | aichat | Trace emission (SPEC-001) | 42A–D **Done** · 42E **Done** (1/2a/2b/3) | [phase-42-overview.md](roadmap/phase-42-overview.md) · [42E](roadmap/phase-42e-overview.md) |
