...
```bash
cargo test --test compatibility tool_execution 2>&1 | grep -E "^test tool_execution::|^test result:" | sed -E "s/finished in [0-9.]+s/finished in Xs/; s/[0-9]+ filtered out/N filtered out/" | sort
```
...
```bash
cargo test --test compatibility 2>&1 | grep 'test result:' | sed -E "s/finished in [0-9.]+s/finished in Xs/; s/[0-9]+ filtered out/N filtered out/; s/[0-9]+ passed/N passed/"
```
