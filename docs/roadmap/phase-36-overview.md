...
**Status (2026-05-26):** **Planned — design draft.** No items below are implemented. This phase closes the incidental divergence identified in Theme 9 of [`260524_anthropic_memory_divergence.md`](https://github.com/jikanter/aichat-private/): pipeline stages share the global `Config` (tools, env, working directory) where Anthropic's sub-agent pattern isolates each. Extends the model-state-restore pattern already in [`src/pipe.rs:164-274`](../../src/pipe.rs) (`run_stage` → `run_stage_inner`) to also restore *config* state across stages, mirroring how [`src/config/resolver.rs:170-185`](../../src/config/resolver.rs) describes macros running in isolated config clones.

| Item | Description | Status |
|---|---|---|
| 36A | Add `config_override: Option<PartialConfig>` field to `PipelineStage` (struct definition at [`src/pipe.rs:18`](../../src/pipe.rs)) and to the YAML stage declaration in `RolePipelineStage` ([`src/config/role.rs`](../../src/config/role.rs)) | Planned (design draft) |
| 36B | Apply override via clone-and-merge — each stage gets a clone of the global `Config` with the override applied; mutations do not leak across stages. Mirrors the macro pattern referenced at [`src/config/resolver.rs:170-185`](../../src/config/resolver.rs). | Planned (design draft) |
| 36C | Preflight validation — reject any `config_override:` that grants tool permissions the parent role does not hold (escalation guard). Extends [`src/config/preflight.rs`](../../src/config/preflight.rs). | Planned (design draft) |
| 36D | Telemetry — extend [`StageTrace`](../../src/pipe.rs) with `config_overrides_applied: Vec<String>` so `-o json` consumers can see what changed per stage | Planned (design draft) |
