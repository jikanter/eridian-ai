...
> ### 37A implementation note (shipped)
>
> - **`CallMetrics` + `ChatCompletionsOutput`** ([`src/client/common.rs`](../../src/client/common.rs)) gained `cache_read_tokens` / `cache_write_tokens`; `CallMetrics::merge` sums them. `SseHandler` ([`src/client/stream.rs`](../../src/client/stream.rs)) gained `set_cache_usage` and a widened `take()` tuple so the streaming path carries cache tokens.
> - **Token convention — full-price remainder (Anthropic-native).** `input_tokens` is the *uncached* prompt remainder; cache tokens are **additive** (grand total = `input + cache_read + cache_write`). The sketch formula's `(input − cache_read − cache_write)` was *not* adopted — it would re-define Anthropic's native field. Instead, OpenAI/Gemini extractors normalize their cache-inclusive `prompt_tokens` into the remainder via the shared `split_cached_prompt` helper. This keeps the emitted `input_tokens` meaning identical across providers (and matches Anthropic's wire shape), which matters because the value is serialized into the SPEC-001 trace and the pi-export shim.
> - **`compute_cost`** is purely additive: `input·ip + cache_read·ip·read_mult + cache_write·ip·write_mult + output·op`. Multipliers default to **0.1 read / 1.25 write** (`DEFAULT_CACHE_READ_MULTIPLIER` / `DEFAULT_CACHE_WRITE_MULTIPLIER`) and are overridable per model via new `cache_read_price_multiplier` / `cache_write_price_multiplier` `ModelData` fields (e.g. set read = 0.5 for OpenAI).
> - **Extractors:** Claude reads `cache_read_input_tokens` / `cache_creation_input_tokens` additively (streaming + non-streaming); OpenAI reads `prompt_tokens_details.cached_tokens`; Gemini/Vertex reads `cachedContentTokenCount` — both via `split_cached_prompt`. Cohere/Bedrock report `None` (no cache reporting wired). The remote-federation path ([`src/config/remote.rs`](../../src/config/remote.rs)) propagates cache tokens from the envelope verbatim.
> - **Coverage:** `compute_cost` discount/write/override + `merge` + `split_cached_prompt` unit tests in `common.rs`; extractor tests in `claude.rs` (additive) and `openai.rs` (split / no-details). **No new CLI/trace surface** (that is 37C `--transparent-cache`, 37D server, 37E `cache.lookup`), so there is no batch/bats/showboat surface to demo at 37A — consistent with how 52A shipped. **Zero new dependencies.**
> - **Carry-over:** the `pi_zero_usage` shim ([`src/config/session.rs`](../../src/config/session.rs)) stays zeroed — it serves *converted historical sessions* that never recorded per-message tokens, a genuine absence of data distinct from the live metrics path.
