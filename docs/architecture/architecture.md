...
The launcher (`src/repl/pi.rs`) mints a localhost bridge port, stages `assets/pi-extensions/aichat-bridge.js` into `<cwd>/.pi/extensions/`, and execs `pi` with `AICHAT_BRIDGE_URL` + `AICHAT_BRIDGE_TOKEN` in the env. Pi owns the TUI; aichat owns inference and state. Slash commands (`/role`, `/agent`, `/macro`, `/rag`, `/session`, `/info`, `/exit-context`) round-trip through `/v1/state/*` on the in-process server, gated by a per-launch bearer token. See [`docs/repl-pi.md`](../repl-pi.md) and [`docs/migrations/pi-repl-migration.md`](../migrations/pi-repl-migration.md).
