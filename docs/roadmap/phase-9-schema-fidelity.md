...
| 9D. Capability-aware pre-flight validation | Done   | Before API call: check `use_tools` vs `supports_function_calling`, images vs `supports_vision`, pipeline stage model availability. Fails at config time, not at API time. Zero tokens. **Scope note:** structural pipeline schema-containment checking (output N satisfies input N+1) is owned by Phase 15A/15B (Contract Testing); 9D covers model-capability mismatches only. |
